...
export default function StockManagement({ 
  ...
  const [viewMode, setViewMode] = useState('grid'); // 'grid' (PrettyLittleThing style) or 'table'
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
          ...
          >
            <Plus size={18} />
            <span>+ Tambah Stok / Baju Baru 👚</span>
          </button>
        </div>
      </div>

      {/* PrettyLittleThing Style Sub-Header Filter Bar */}
        ...
        <div style={{ 
          ...
          {filteredProducts.length === 0 ? (
            <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: '4rem', background: '#ffffff', borderRadius: 'var(--radius-md)', border: '2px dashed #ffccd5' }}>
              ...
              <div style={{ fontSize: '1.1rem', fontWeight: '700', color: '#ff4d6d' }}>Tiada produk dijumpai 🌸</div>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginTop: '0.3rem' }}>Sila cuba carian atau penapis lain.</p>
            </div>
          ) : (
            filteredProducts.map((product) => {
              ...
              // Default avatar placeholder image if no custom image
              ...
              return (
                ...
                >
                  ...
                  <div style={{ 
                    ...
                    {/* Top Right Floating Action Badge (+) */}
                    <button
                      onClick={() => onOpenMovementModal('STOCK_IN', product)}
                      style={{
                        position: 'absolute',
                        top: '10px',
                        right: '10px',
                    ...
                    {/* Status Badge Overlaid on Image (PrettyLittleThing Style) */}
                  ...
                  {/* Product Details Section (PrettyLittleThing Minimalist Typography) */}
                  <div style={{ padding: '1.1rem', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                    <div>
                      ...
                      {/* Product Title (PrettyLittleThing All-Caps Style) */}
                      <h4 style={{ 
                        fontSize: '0.92rem', 
                        fontWeight: '700', 
                        textTransform: 'uppercase', 
                      ...
                      {/* Price Tag (PrettyLittleThing Price Styling) */}
                    ...
                    {/* Card Footer Actions */}
                    <div style={{ display: 'flex', gap: '0.4rem', borderTop: '1px dashed #f0e6e1', paddingTop: '0.75rem' }}>
                      <button 
                        className="btn btn-success btn-sm"
                        disabled={isSoldOut}
                        onClick={() => onOpenMovementModal('STOCK_OUT', product)}
                        style={{ flex: 1, padding: '0.4rem', opacity: isSoldOut ? 0.4 : 1 }}
                        title="Rekod Jualan"
                      >
                        <ArrowUpRight size={14} /> Jual 🛍️
                      </button>
                    ...
                    <tr key={product.id}>
                      ...
                      <td>
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.4rem' }}>
                          <button className="btn btn-success btn-sm" disabled={isSoldOut} onClick={() => onOpenMovementModal('STOCK_OUT', product)}>
                            Jual 🛍️
                          </button>
                          <button className="btn btn-primary btn-sm" onClick={() => onOpenMovementModal('STOCK_IN', product)}>
                            Restock 🚚
                          </button>
